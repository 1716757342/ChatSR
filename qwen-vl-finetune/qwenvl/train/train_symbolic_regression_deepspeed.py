...
def train_symbolic_regression_deepspeed(attn_implementation="flash_attention_2"):
    ...
    rank0_print("🚀 开始符号回归模型训练 (DeepSpeed优化版本)...")
    ...
    # 使用简化的模型加载策略
    rank0_print("📋 DeepSpeed模式：直接创建符号回归模型...")
    ...
    # 在DeepSpeed模式下，让训练器自动处理权重加载
    rank0_print("📋 让DeepSpeed自动处理模型初始化...")
